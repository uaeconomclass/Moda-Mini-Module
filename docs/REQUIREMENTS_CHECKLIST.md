...
2. Uses batch inserts (chunked) for scale.
3. Link seeding uses `INSERT IGNORE` to respect unique constraint.

Performance:

1. List endpoint is paginated with `LIMIT/OFFSET`.
2. Celebrity filtering uses SQL joins, no per-row N+1 loading.
3. `celebrity_count` is computed in SQL aggregation.
4. Indexes documented in `README.md`.
