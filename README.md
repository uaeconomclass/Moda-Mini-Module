...
2. **Stylist Detail**
1. Shows stylist fields
2. Lists reps and celebrities
3. Add/delete rep
4. Attach/detach celebrity
3. **Celebrities List**
...
2. Celebrity filtering uses indexed `JOIN`s:
...
3. Avoiding N+1:
1. List endpoint computes `celebrity_count` via `LEFT JOIN` + `COUNT(DISTINCT ...)` + `GROUP BY`.
...
3. Add proper admin list table classes and better bulk operations.
4. Add transaction/batch insert strategy for much faster very large seeding.
5. Add custom capability + roles instead of `manage_options` only.
